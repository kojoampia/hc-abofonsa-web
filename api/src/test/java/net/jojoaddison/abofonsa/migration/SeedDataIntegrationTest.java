...
import net.jojoaddison.abofonsa.content.FaqRepository;
import net.jojoaddison.abofonsa.content.PlanRepository;
import net.jojoaddison.abofonsa.content.SectionRepository;
import net.jojoaddison.abofonsa.content.ServiceRepository;
import net.jojoaddison.abofonsa.content.SiteSettingsRepository;
import net.jojoaddison.abofonsa.content.TestimonialRepository;
...
class SeedDataIntegrationTest extends AbstractIntegrationTest {
    ...
    void bootstrapAdminSeededWithMustChangePassword() {
        var admin = mongoTemplate.findAll(AdminUserDocument.class).stream()
                .filter(u -> u.username().equals("admin"))
                .findFirst()
                .orElseThrow();
        assertThat(admin.mustChangePassword()).isTrue();
        assertThat(admin.roles()).containsExactly(AdminUserDocument.Role.ADMIN);
