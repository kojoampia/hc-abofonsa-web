    ...
    @if (section(); as life) {
      <section class="py-16 bg-brand-cream" aria-labelledby="careers-life-heading">
        <div class="max-w-6xl mx-auto px-4 prose-reset flex flex-col gap-6">
          @if (life.eyebrow) {
            <p abcContentLang class="text-brand-gold-ink text-sm uppercase tracking-wide">{{ life.eyebrow }}</p>
          }
          <h2 abcContentLang id="careers-life-heading" class="font-serif text-3xl text-brand-navy">{{ life.heading }}</h2>
          @if (life.subheading) {
            <p abcContentLang class="text-brand-body max-w-2xl">{{ life.subheading }}</p>
          }
          <div class="grid md:grid-cols-2 gap-x-10 gap-y-6 mt-2">
            @for (item of life.items; track item.key) {
              <div abcContentLang class="prose-reset">
