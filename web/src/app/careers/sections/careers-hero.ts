    ...
    @if (section(); as hero) {
      <section class="bg-brand-navy text-white" aria-labelledby="careers-heading">
        <div class="max-w-6xl mx-auto px-4 py-16 lg:py-24 prose-reset flex flex-col gap-5 max-w-3xl">
          ...
          <h1 id="careers-heading" class="font-serif text-4xl lg:text-5xl leading-tight">{{ hero.heading }}</h1>
          @if (hero.subheading) {
            <p class="text-lg text-white/80">{{ hero.subheading }}</p>
          }
          @if (hero.body) {
            <p class="text-white/70 leading-relaxed">{{ hero.body }}</p>
          }
