...
@Component({
  ...
  imports: [TranslocoPipe, CareersCta, ContentLang],
  template: `
          ...
          @for (track of store.tracks(); track track.id) {
            ...
            >
              <div class="flex items-start gap-3 flex-wrap">
                <h3 abcContentLang class="font-serif text-2xl text-brand-navy flex-1">{{ track.title }}</h3>
                @if (track.openings) {
              ...
              <p abcContentLang class="text-brand-body leading-relaxed">{{ track.blurb }}</p>
              ...
              @if (track.requirements.length) {
                <div>
                  ...
                  <ul abcContentLang class="grid gap-1 text-sm text-brand-body list-none p-0">
